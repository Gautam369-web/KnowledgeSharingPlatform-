...
const MOCK_LEADERS = [
    { rank: 1, name: 'Alex Johnson', reputation: 15420, solved: 234, badge: '🥇', color: '#d4a017' },
            ...
            <div style={{ background: '#0e2010', borderBottom: '1px solid rgba(74,158,92,0.12)', padding: '40px 24px 32px' }}>
                <div style={{ maxWidth: 900, margin: '0 auto' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                        <HiOutlineChartBar style={{ color: '#d4a017', width: 20, height: 20 }} />
                        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', color: '#d4a017' }}>LEADERBOARD</span>
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 32 }}>
                    {MOCK_LEADERS.slice(0, 3).map((u, i) => (
                        <div key={u.rank} style={{
                            background: i === 0 ? 'linear-gradient(135deg,rgba(212,160,23,0.12),rgba(212,160,23,0.04))' : 'rgba(255,255,255,0.02)',
                            border: i === 0 ? '1px solid rgba(212,160,23,0.35)' : '1px solid rgba(74,158,92,0.14)',
                            borderRadius: 16, padding: '28px 20px', textAlign: 'center',
                            order: i === 0 ? -1 : i,
                        }}>
                            <div style={{ fontSize: 32, marginBottom: 10 }}>{u.badge}</div>
                            <div style={{ width: 48, height: 48, borderRadius: '50%', background: `linear-gradient(135deg,${u.color},${u.color}80)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800, color: '#0a1a0d', margin: '0 auto 12px' }}>
                                {u.name[0]}
                            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {MOCK_LEADERS.slice(3).map((u) => (
                        <div key={u.rank} className="card" style={{ padding: '18px 24px', display: 'flex', alignItems: 'center', gap: 20 }}>
                            ...
                            <div style={{ width: 38, height: 38, borderRadius: '50%', background: `linear-gradient(135deg,${u.color},${u.color}70)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800, color: '#0a1a0d' }}>{u.name[0]}</div>
                            <div style={{ flex: 1 }}>
